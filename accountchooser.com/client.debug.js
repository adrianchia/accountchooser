...
//java/com/google/apps/easyconnect/easyrp/widget/javascript/cds/client-source.js
...
/**
 * @fileoverview Utility function to detect whether the browser is SNI (Server
 * Name Indicator) supported.
 * @author mengcheng@google.com (Mengcheng Duan)
 */
 ...
 * }
 * ...
 * @param {boolean=} opt_lookahead If set to {@code true}, it will do look-ahead
 *     checking to decide whether to redirect or not.
 * </pre>
 */
window.accountchooser.CdsClient.prototype.store = function(
    account, localAccounts, opt_cdsOptions, opt_lookahead) {
  window.accountchooser.param.notEmpty(account, 'account');
  ...
  if (!opt_lookahead) {
    ...
    var callback = this.createLookAheadQueryCallback_(store,
        function(queryResult) {
          return queryResult;
        },
        {stored: true});
    this.checkAccountExist(account, callback, options);
  }
};
...
window.accountchooser.CdsClient.prototype.select = function(
    localAccounts, opt_cdsOptions, opt_lookahead) {
  var service = 'select';
  var options = opt_cdsOptions ?
      window.accountchooser.util.extend(false, {},
          this.cdsOptions_, opt_cdsOptions) :
  ...
  if (!opt_lookahead) {
    ...
    var callback = this.createLookAheadQueryCallback_(select,
        function(queryResult) {
          return queryResult && !(localAccounts && localAccounts.length);
        },
        {addAccount: true});
    this.checkCdsEmpty(callback, options);
  }
};
...
window.accountchooser.CdsClient.prototype.update = function(
    account, opt_cdsOptions, opt_lookahead) {
  window.accountchooser.param.notEmpty(account, 'account');
  ...
  if (!opt_lookahead) {
    ...
    var callback = this.createLookAheadQueryCallback_(update,
        function(queryResult) {
          return !queryResult;
        },
        {updated: false});
